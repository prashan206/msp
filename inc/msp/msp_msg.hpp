...
struct SetPositionEstimationConfig : public PositionEstimationConfigSettings,
    ...
    virtual ByteVectorUptr encode() const override {
        ByteVectorUptr data = std::make_unique<ByteVector>();
        bool rc             = true;
        rc &= data->pack<uint16_t>(w_z_baro_p, 0.01);
        rc &= data->pack<uint16_t>(w_z_gps_p, 0.01);
        rc &= data->pack<uint16_t>(w_z_gps_v, 0.01);
        rc &= data->pack<uint16_t>(w_xy_gps_p, 0.01);
        rc &= data->pack<uint16_t>(w_xy_gps_v, 0.01);
...
struct Attitude : public Message {
    ...
    Value<int16_t> roll;   // [-180, +180] degree
    Value<int16_t> pitch;  // [-90, +90] degree
...
struct Analog : public Message {
    ...
    virtual bool decode(const ByteVector& data) override {
        ...
        rc &= data.unpack(rssi);
        rc &= data.unpack<int8_t>(amperage, 10);
        return rc;
    }
